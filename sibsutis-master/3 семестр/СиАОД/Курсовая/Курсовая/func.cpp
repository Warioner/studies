...
void digitalSort(listDataBase *(&S)) {
  int KDI[32];
  for (int i = 0; i < 30; i++)
    KDI[i] = i;
  KDI[30] = 31;
...
void quickSortCoding(
    coding *A, int R,
    int L) // Сортировка для кодировки (сортировка по убыванию вероятности)
{
  while (L < R) {
...
// Находит медиану части массива P, т.е. такой индекс L <= m <= R, что
// минимальна величина
...
void clearBuffer() {
  FlushConsoleInputBuffer(
      GetStdHandle(STD_INPUT_HANDLE)); // Очищает буфер консоли
